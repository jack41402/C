...
int main (int argc , char *argv[])
{
    ...
    int fillings ;
    int pos , filter ;
    int first_filter , bandwidth , weight ;
    if (argc==1)
    {
    ...
    for (int i=0 ; i<header.Height ; ++i)
    {
        ...
        {
            pos = i * rowSize + j * 3 ;
            filter = (j/bandwidth + first_filter) % 4 ;
            switch (filter)
            {
                case 0 :
                {
                ...
                case 1 :
                {
                    imageData[pos] = (imageData[pos]<255-weight) ? imageData[pos]+weight : weight ;
                    break ;
                }
                case 2 :
                {
                    imageData[pos+1] = (imageData[pos+1]<255-weight) ? imageData[pos+1]+weight : weight ;
                    break ;
                }
                case 3 :
                {
                    imageData[pos+2] = (imageData[pos+2]<255-weight) ? imageData[pos+2]+weight : weight ;
                    break ;
                }
                default :
                {
