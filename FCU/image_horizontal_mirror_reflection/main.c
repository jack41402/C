...
int main (int argc , char *argv[])
{
    ...
    for (int i=0 ; i<header.Height/2 ; ++i)
    {
        ...
        {
            pos = i * rowSize + j * 3 ;
            reflect_pos = (header.Height - i - 1) * rowSize + j * 3 ;
            swap(imageData[pos] , imageData[reflect_pos]) ;
            swap(imageData[pos+1] , imageData[reflect_pos+1]) ;
            swap(imageData[pos+2] , imageData[reflect_pos+2]) ;
        }
    }
    ...
    // Print the horizontal_mirror_reflection image file head.
