...
void Write_data (char *fname, Header header, unsigned char *palette, unsigned char *imageData)
{
    ...
    fwrite(&header.Colors, 4, 1,
           fptr); // Value is zero except for indexed images using fewer than the maximum number of colors.
    fwrite(&header.ImportantColors, 4, 1,
           fptr); // Number of colors that are considered important when rendering the image.
...
int main (int argc, char *argv[])
{
    ...
    palette = (unsigned char *) malloc(
            heaori.OffsetBits - heaori.InfoSize - 14); // Allocate memory space for the palette.
...
//  Write_data(argv[1],heaori,palette,imageData);
    print_data(heaori);
    ...
    for (i = 0; i<heared.Height/2; i++)
    {
        for (k = 0; k<heared.Width/2; k++)
        {
    ...
    Write_data(argv[2], heared, palette, reimageData);
    ...
    for (i = 0; i<heafin.Height; i++)
    {
        for (k = 0; k<heafin.Width; k++)
        {
            ...
            if (k>=0 && k<n || k>=heared.Width + n && k<heared.Width + 2 * n ||
                k>=heared.Width * 2 + 2 * n && k<heared.Width * 2 + 3 * n)
            {
                finimageData[kfin] = fcol3;
                finimageData[kfin + 1] = fcol2;
                finimageData[kfin + 2] = fcol1;
            }
            else if (i>=0 && i<n || i>=heared.Height + n && i<heared.Height + 2 * n ||
                     i>=heared.Height * 2 + 2 * n && i<heared.Height * 2 + 3 * n)
            {
                finimageData[kfin] = fcol3;
                finimageData[kfin + 1] = fcol2;
                finimageData[kfin + 2] = fcol1;
            ...
            else if (i>=heared.Height + 2 * n && i<heared.Height * 2 + 2 * n && k>=n && k<heared.Width + n)
            {
                ...
                finimageData[kfin] = reimageData[kr + (heared.Height - (i - 2 * n - heared.Height) * 2) * rerowsize];
                finimageData[kfin + 1] = reimageData[kr +
                                                     (heared.Height - (i - 2 * n - heared.Height) * 2) * rerowsize + 1];
                finimageData[kfin + 2] = reimageData[kr +
                                                     (heared.Height - (i - 2 * n - heared.Height) * 2) * rerowsize + 2];
            }
            else
            {
                kr = (i - 2 * n - heared.Height) * rerowsize + (k - 2 * n - heared.Width) * 3;
                finimageData[kfin] = reimageData[kr + (heared.Height - (i - 2 * n - heared.Height) * 2) * rerowsize +
                                                 (heared.Width - (k - 2 * n - heared.Width) * 2) * 3];
                finimageData[kfin + 1] = reimageData[kr +
                                                     (heared.Height - (i - 2 * n - heared.Height) * 2) * rerowsize +
                                                     (heared.Width - (k - 2 * n - heared.Width) * 2) * 3 + 1];
                finimageData[kfin + 2] = reimageData[kr +
                                                     (heared.Height - (i - 2 * n - heared.Height) * 2) * rerowsize +
                                                     (heared.Width - (k - 2 * n - heared.Width) * 2) * 3 + 2];
            }
        }
    }
    Write_data(argv[3], heafin, palette, finimageData);
